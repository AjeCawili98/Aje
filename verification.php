		...
		switch(strtolower($_POST['action'])){
			case 'activation-sendemail':
				...
				if(phpMailer($email, "Activation Code", $message)){
					...
					insertActivation($username, $activationCode);
				} else {
			...
			case 'activate':
				...
				$checkActivation = "select * from account_activations where username = " . $username . " and activation_key = '" . $activationCode . "'";
				$rsActivation = mysqli_query($mysqli, $checkActivation);
				$cntActivation = mysqli_num_rows($rsActivation);

				if ($cntActivation > 0) {
					$updateUser = "update users set status_id = 1 where id = " . $username;
					$rsUpUser = mysqli_query($mysqli, $updateUser);

					$deleteActivation = "delete from account_activations where username = " . $username . " and activation_key = '" . $activationCode . "'";
